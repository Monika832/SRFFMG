...
######### 
    ...
    if tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_{}.npz'.format('ModifiedSRGAN'), network=net_g) is False:
        tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/g_{}_init.npz'.format('ModifiedSRGAN'), network=net_g)
    tl.files.load_and_assign_npz(sess=sess, name=checkpoint_dir+'/d_{}.npz'.format('ModifiedSRGAN'), network=net_d)
    ###============================= LOAD VGG ===============================###
...
#            
            ...
            if (iters != 0) and (iters % 1000 == 0):
                tl.files.save_npz(net_g.all_params, name=checkpoint_dir+'/g_{}.npz'.format('ModifiedSRGAN'), sess=sess)
            if (iters != 0) and (iters % 10000 == 0):                
                tl.files.save_npz(net_d.all_params, name=checkpoint_dir+'/d_{}.npz'.format('ModifiedSRGAN'), sess=sess)
