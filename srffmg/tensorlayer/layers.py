...
def custom_pad(inputs, filter_shape, strides, padding_type, name):
    #NHWC
    tensor_shape = inputs.get_shape()
    in_height = tensor_shape[1].value
    in_width = tensor_shape[2].value
    filter_height = filter_shape[0]
    filter_width = filter_shape[1]
    
#    if (in_height % strides[1] == 0 ):
#    pad_along_height = tf.maximum(filter_height - strides[1], 0)
    pad_along_height = filter_height - strides[1]
#    else:
#      pad_along_height = tf.maximum(filter_height - (in_height % strides[1]), 0)

#    if (in_width % strides[2] == 0):
#    pad_along_width = tf.maximum(filter_width - strides[2], 0)
    pad_along_width =filter_width - strides[2]
#    else:
#      pad_along_width = tf.maximum(filter_width - (in_width % strides[2]), 0)  
    pad_top = pad_along_height // 2
    pad_bottom = pad_along_height - pad_top
    pad_left = pad_along_width // 2
    pad_right = pad_along_width - pad_left        
    
    outputs = tf.pad(inputs, [ [0,0], [pad_top, pad_bottom], [pad_left, pad_right], [0,0] ], padding_type, name=name)
    return outputs
        ...
        with tf.variable_scope(name) as vs:
            ...
            if b_init:
                ...
                if(padding=='SYMMETRIC'):
                    self.inputs = custom_pad(self.inputs, shape, strides, padding, name = 'pad')
                    self.outputs = act( tf.nn.conv2d(self.inputs, W, strides=strides, padding='VALID', use_cudnn_on_gpu=use_cudnn_on_gpu, data_format=data_format) + b )
                else:
...
## Super resolution
def SubpixelConv2d_sj(net, scale=2, n_out_channel=None, act=tf.identity, name='subpixel_conv2d'):
    ...
    def _PS(X, r, n_out_channel):
        if n_out_channel >= 1:
            ...
            '''
            Xs = tf.split(X,n_out_channel,3)
            outputs = []
            for i in range(n_out_channel):
                Xt=tf.depth_to_space(Xs[i],r)
                outputs.append(Xt)
...
#        print("  [TL] FlattenLayer %s:" % (self.name))        
        self.all_layers = list(layer.all_layers)
        self.all_params = list(layer.all_params)
        self.all_drop = dict(layer.all_drop)
        self.all_layers.extend( [self.outputs] )
